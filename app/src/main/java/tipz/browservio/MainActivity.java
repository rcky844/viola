	...
	private void initialize(Bundle _savedInstanceState) {
		...
		browse.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				if (urledit.getText().toString().equals("browservio://nothing") || urledit.getText().toString().equals("browservio://empty")) {
					...
					if(URLUtil.isValidUrl(urledit.getText().toString())) {
						...
						webview.loadUrl("https://www.google.com/search?q=".concat(urledit.getText().toString()));
					}
				}
			}
		});
		...
		linear_control_b1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				if (webview.canGoForward()) {
					...
					// cannot go forward
					SketchwareUtil.showMessage(getApplicationContext(), "Cannot go forward");
				}
			}
		});
...
Current default page: google.com */
	...
	private void _useragentSet (final WebView _webview, final String _ua) {
		_webview.getSettings().setUserAgentString(_ua);
	}
